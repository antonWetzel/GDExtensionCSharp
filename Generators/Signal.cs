	...
	public static class Signal {
		...
		public static void Generate(GeneratorExecutionContext context, INamedTypeSymbol c) {
			...
			for (var i = 0; i < events.Length; i++) {
				...
				code += $"\t\tGDExtensionInterface.gdInterface.classdb_register_extension_class_signal(GDExtensionInterface.gdLibrary, __godot_name._internal_pointer, new StringName(\"{Renamer.ToSnake(ev.Name)}\")._internal_pointer, {infosName}, {m.Parameters.Length});\n";
			}
			code += $$"""
				}
